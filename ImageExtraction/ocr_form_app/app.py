...
def _do_fill(driver, form_url, data_dict):
    ...
    # SAFETY CHECK (fixes the "idle browser dies -> file still deleted" bug):
    # if the internal system's session had expired (e.g. left idle too
    # long), driver.get(form_url) silently lands back on the LOGIN page
    # instead of the real form. The login page also has <input> tags, so
    # the WebDriverWait above still succeeds -- but there are only 2 of
    # them (username/password), while the form needs 31. Previously this
    # made every field beyond index 1 throw IndexError, which was CAUGHT
    # and only printed, so _do_fill() returned as if nothing went wrong,
    # fill_form() reported success, and review() then deleted the
    # original + working copy of a form that was never actually filled.
    # Now we fail loudly and early instead, before anything is deleted.
    expected_field_count = 31
    if len(inputs) < expected_field_count:
        raise RuntimeError(
            f"Expected {expected_field_count} fields on the form but only found "
            f"{len(inputs)} input boxes (current page: {driver.current_url}). "
            "This usually means the browser session had expired/logged out and "
            "landed on a different page instead of the real form. Nothing was "
            "filled or deleted -- please try again."
        )
...
def review():
    if request.method == "POST":
        ...
        # Previously this returned a plain text message and just sat there,
        # so you had to manually retype the URL to get back to the upload
        # page for the next record. Now it redirects there automatically,
        # with a small banner (via ?filled=1) confirming it went through.
        return redirect(url_for("upload_file", filled=1))
...
def _open_browser_once():
    """Automatically opens the app in the default browser once the server
    is up, so a non-technical person never has to know/type the localhost
    URL themselves -- just double-click the launcher and the page appears.
    Flask's debug reloader actually starts TWO processes; WERKZEUG_RUN_MAIN
    is only set to 'true' inside the real (reloaded) worker, so this check
    stops the browser from popping open twice."""
    if os.environ.get("WERKZEUG_RUN_MAIN") == "true":
        webbrowser.open("http://127.0.0.1:5000/")


if __name__ == "__main__":
    threading.Timer(1.5, _open_browser_once).start()
    app.run(debug=True)
